 ...
 * Diese Klasse kapselt Swing, Tastatur und Zeichnen.
 * Sie ist Unterrichtsinfrastruktur und muss von Lernenden nicht verändert werden.
 */
public class SpaceWindow {
    ...
    private static class SpacePanel extends JPanel {
        ...
        private void drawPlanetShape(Graphics2D g2,PlanetView p){int r=(int)Math.round(p.radius),x=(int)Math.round(p.x-r),y=(int)Math.round(p.y-r);g2.setColor(new Color(85,105,190));g2.fillOval(x,y,2*r,2*r);g2.setColor(new Color(170,190,255));g2.drawOval(x,y,2*r,2*r);g2.setColor(new Color(220,230,255));g2.setFont(new Font(Font.SANS_SERIF,Font.PLAIN,12));g2.drawString(p.name,x,y-6);}
